...
3. What is the purpose of the functions?
    ...
    - `infraction_edit@infraction/management.py` modifies punishments for users who have violated server rules and notifies moderators about the changes. It is used by `infraction_append`, which applies new punishments, since the two functions share most logic.

      The function must handle various edge cases, such as preventing edits to expired infractions or warnings. It also processes multiple input formats and validates the request before making API calls. These requirements introduce multiple decision points, contributing to its high CC.
...
- Refactoring `infraction_edit@infraction/management.py` should decrease the CC from 18 to 11, which is a decrease of about 39%. No drawbacks are anticipated.
...
- [PR #21](https://github.com/dd2480-spring-2025-group-1/bot/pull/21) is a carried out refactoring of `infraction_edit@infraction/management.py` which reduces the CCN by about 39%.
...
Branch where coverage tool was implemented for`infraction_edit@infraction/management.py`: [infraction_edit](https://github.com/dd2480-spring-2025-group-1/bot/pull/28)
...
- For `infraction_edit@infraction/management.py`, there were no tests before but the documentation of the function was clear and helped in creating the requirements:
    ...
    - The function should call the `api_client.patch` method
